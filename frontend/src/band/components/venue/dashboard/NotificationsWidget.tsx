    ...
    <Card className="bg-card/45 backdrop-blur-md border border-border shadow-xl h-full">
      ...
      <CardContent className="p-4 space-y-3 max-h-[350px] overflow-y-auto pr-1">
        {notifications.map((notif) => {
          const isWarning = notif.title.toLowerCase().includes("warn") || notif.title.toLowerCase().includes("request") || notif.title.toLowerCase().includes("enquiry");
          ...
          return (
            <div 
              key={notif.id} 
              className={`p-3 rounded-xl border border-border bg-accent/10 flex gap-3 hover:border-primary/40 transition-colors ${!notif.is_read ? "border-primary/30 bg-primary/5" : ""}`}
            >
              ...
              <div className="space-y-0.5 flex-1">
                <div className="flex justify-between items-start gap-2">
                  <span className={`text-xs font-bold text-foreground block ${!notif.is_read ? "text-primary" : ""}`}>
                    {notif.title}
                  </span>
